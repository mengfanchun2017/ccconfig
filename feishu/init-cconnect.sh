...
lines.append('# 由 init-cconnect.sh 自动生成，请勿手动编辑')
lines.append('# 修改用户配置请编辑 conf/feishu.json 后重新运行 init-cconnect.sh')
lines.append('')
lines.append('[log]')
...
main() {
    ...
    echo "  bash ccconfig/feishu/init-cconnect.sh  # 重新配置（修改 conf/feishu.json 后）"
    echo ""
    echo "添加新用户："
    echo "  1. 编辑 ../conf/feishu.json，在 ccconnect.users 中新增用户"
    echo "  2. 在飞书开放平台创建新的企业自建应用"
    echo "  3. 运行 bash ccconfig/feishu/init-cconnect.sh 重新生成配置"
    echo ""
}
