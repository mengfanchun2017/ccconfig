#!/bin/bash
# Bridge 初始化脚本
# 功能：安装配置 ccbot (飞书 Bridge WebSocket 长连接)
# 配置：从 conf-feishu.json 读取
# 目的：仅在长期运行 Bridge 的那台机器上运行（非所有环境）
#
# 使用：
...
try:
    with open(sys.argv[1], 'r', encoding='utf-8') as f:
        data = json.load(f)
    print(data.get('ccbot', {}).get('workDir', '/home/francis/git'))
except:
    print('/home/francis/git')
PYEOF
}
...
main() {
    echo -e "${CYAN}"
    echo "╔══════════════════════════════════════════════════╗"
    echo "║         Bridge 初始化 - Bridge Init             ║"
    echo "║    仅在长期运行 Bridge 的那台机器上运行          ║"
    echo "╚══════════════════════════════════════════════════╝"
    echo "$NC"
