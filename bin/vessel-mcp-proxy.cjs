...
const MCP_TIMEOUT_MS = parseInt(process.env.VESSEL_MCP_TIMEOUT_MS || "30000", 10);
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = http.request(
      ...
      (res) => {
        ...
        res.on("error", (err) => {
          if (!timedOut) {
            emitHttpError(body.id ?? null, null, err.message);
          }
          once(() => reject(err));
        });
      },
    ...
    req.on("timeout", () => {
      ...
      writeMessage(createJsonRpcError(
        body.id ?? null,
        `Vessel MCP timeout after ${MCP_TIMEOUT_MS}ms — browser unresponsive, GPU likely stuck. Run: vessel-healthcheck.sh --fix`,
        -32000));
      once(() => resolve());
    });
