...
| pullff `[repo]` | `bash maintain.sh sync --pull` 强拉远程 → 自动重建符号链接 |

## 常用命令
- 运维入口: `bash maintain.sh` (status/monitor/daemon/update/sync/deps/fix)
- 初始化入口: `bash init.sh`
...
```
`bootstrap.sh` 自动装 git → clone ccconfig → 输出下一步命令。完整流程看 `BOOTSTRAP.md`。
