...
```bash
# Search by filename
find "<your-obsidian-vault-path>/" -name "*.md" | grep -i "keyword"

# Search by content
grep -rl "keyword" "<your-obsidian-vault-path>/" --include="*.md"
```
...
```bash
grep -rl "\\[\\[Note Title\\]\\]" "<your-obsidian-vault-path>/"
```

### Find index notes

```bash
find "<your-obsidian-vault-path>/" -name "*Index*"
```
