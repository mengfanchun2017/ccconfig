...
2. **gitforce.sh**：执行 `cd "$REPO_DIR"` 后 session cwd 变为 ccconfig 目录

## 影响

- 后续 `git diff gitforce.sh` 报错 `Could not access 'gitforce.sh'`（因为 cwd 已变）
- 相对路径全部失效

## 解决方向

1. **避免 cd**：所有脚本不主动 cd，改用绝对路径或 `git -C <dir>`
2. **Bash 命令前重置 cwd**：在复杂命令前加 `cd $HOME/git &&` 确保目录正确
3. **使用绝对路径引用脚本**：不用相对路径

## 已更新的脚本（2026-05-15）

- gitforce.sh：pullff 后同步 skills（已在末尾）
- monitor.sh：auto-sync 后同步 skills（已在 pull 成功后）
