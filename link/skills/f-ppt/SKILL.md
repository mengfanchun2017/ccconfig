...
### Step 3.5 — 字号与间距规范（硬约束，避免文字溢出）

以下为 1280×720 px SVG 画布上的数值，经 professional_blue 模板实战验证。所有内容页生成**必须**遵守此表，不得超出上限。

| 元素 | 字号 px | 形状最小高度 | 内边距 | 说明 |
|------|---------|-------------|--------|------|
| 页面标题 | 28–30 | bar 72px | 上 24px | `y = bar_center + font × 0.35` |
| Section 标题条 | 20 | bar 36px | 上 12px | 单行文字 + 蓝底反白 |
| 卡片标题 | 20 | — | — | 在卡片内第一行 |
| 正文 | **18** | — | 行距 ≥ 28px | **硬上限**，不可 > 18px |
| 正文（密集） | 16 | — | 行距 ≥ 24px | 表格、列表等高密度处 |
| 注释/脚注 | 13–14 | — | — | 灰色、底部 |
| 表格文字 | 17–18 | 行高 ≥ 60px | 左 12px 上 22px | 列宽 ≥ 汉字数 × font × 1.1 |
| KEY MESSAGE 条 | 22 | bar 42px | 上 14px | 单行，强调色 |

**中文列宽公式**：`col_min_width = 汉字数 × font_size × 1.15`。例如「单机月电费（50%/额定）」= 12 字 × 18px × 1.15 ≈ 248px → 列宽 ≥ 250px。中文比西文宽约 15%，公式中的 1.15 是安全系数。

**卡片行数上限**：
- 宽卡（543px）：≤ 5 行正文（每行 ~28 字），h ≥ 230px
- 窄卡（355px）：≤ 3 行正文（每行 ~18 字），h ≥ 96px

**禁止行为**：
- ❌ 正文 > 18px（必然溢出）
- ❌ 表格列宽 < 汉字数 × font × 1.1（中文截断或换行）
- ❌ 卡片内边距 < 12px（文字贴边）
- ❌ 在未验证形状高度的情况下增加行数
- ❌ 启用 autofit 后处理（编辑时字体跳变，见 [§文本溢出](#文本溢出autofit)）
