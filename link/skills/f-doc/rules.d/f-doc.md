...
## 封面图选择（2026-06-09 用户明确偏好）

| 场景 | 选 | Why |
|---|---|---|
| 飞书内分享 / 编辑 / 留痕 | **白板**（mermaid 源 + `whiteboard +update`）| 飞书 UI 可直接改、体现「飞书画图」语义 |
| 跨平台（README / 博客 / 微信 / 邮件）| **PNG**（matplotlib 生成）| 通用格式，无飞书访问依赖 |
| 同时需要 | **白板 + PNG 双存** | 白板主用，PNG 作 fallback |

**禁止**：用 PNG 当 doc 封面图（用户原话「飞书画图」= 必须可编辑）
**白板一定在 title 之后**（无法插到 title 之前）
