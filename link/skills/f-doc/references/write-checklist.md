...
> **唯一真相源** — f-doc 编排层每次写操作前逐项核对。整合全部格式约束、更新安全规则、验证步骤。
> 全局格式硬约束（不调 f-doc 也生效）→ `rules.d/f-doc.md`
...
- [ ] **跨租户？** 不同域名（`<your-tenant>.feishu.cn` vs `acimdomc.feishu.cn`）→ lark-cli 无法访问，需导出本地处理
