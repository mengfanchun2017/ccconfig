...
| `wsl-interop.ps1` | PS | 写 `/etc/wsl.conf`，关 Windows PATH 注入（内部用 `wsl.exe -u root`） |
...
# Win 端：写 /etc/wsl.conf（关 Win PATH 注入；脚本内部用 wsl.exe -u root）
powershell.exe -ExecutionPolicy Bypass -File "$(wslpath -w ccconfig/windows-tools/wsl-interop.ps1)"
...
- `wsl-interop.sh` → `wsl-interop.ps1`（同次会话后续）：写 `/etc/wsl.conf` 虽然改 WSL 文件，但 `wsl.exe -u root` 从 PowerShell 调用更干净；目录全部 PS 统一，不再混 .sh
