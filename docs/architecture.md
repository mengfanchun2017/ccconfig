...
| **claude-skills** | 公开 | 12 个 f-* skill 插件 marketplace | clone 或 `/plugin marketplace add` |
...
│   ├── init-skill.sh           # Skills 同步（3 阶段 pipeline）
...
每次 Claude Code 启动自动运行（SessionStart hook）。11 项检查：
...
Layer 2: 编排（委托 Layer 1）
  f-doc            飞书文档统一入口 → 委托 f-pdf/f-ppt/lark-cli
  f-ppt            PPT 生成 → 委托 OfficeCLI
  f-research       快速研究 → 委托 f-search
  f-logme          个人管理 → 委托 f-doc

Layer 3: 复合工作流
  f-research-deep      批量深度研究
  f-research-report    报告生成 → 委托 f-research + f-report-std + f-doc
  f-launch             项目启动 → 委托 f-logme + f-doc
...
| 来源 | 安装方式 | 管理 |
|------|---------|------|
| 自建 f-*（12 个） | `init-skill.sh sync` 从 claude-skills symlink | ccconfig |
| 第三方（mattpocock） | `npx skills add` 从 GitHub | `conf/third-party-skills.txt` |
| 私有覆盖 | `apply-config.sh` ln -s ccprivate/config/*.yaml | ccprivate |

### 私有配置覆盖

skill 的 `config.yaml` 实际是 symlink → `ccprivate/config/<skill>.yaml`。skill 内 Python 脚本 `open('config.yaml')` 自动跟踪 symlink 读到真实 token。修改 ccprivate 立即生效。
...
| Skill 私有配置 | ccprivate/config/*.yaml | 私有仓库 |
...
4. 如有私有配置：`ccprivate/config/<name>.yaml` → `apply-config.sh` 自动覆盖
