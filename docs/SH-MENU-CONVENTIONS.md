...
- 选项 `1A` 绿色加粗，无空格
- 分类标题 `--状态--` 灰色加粗
- `0` 退出行绿色加粗（`  ` + `0` + `  退出`），文案 `退出`；与子菜单 `返回上层` 区分
- 提示 `选择:` 绿色加粗
- 分类编号连续无跳号（1,2,3...不跳 2→4）
...
```
  ...
  选择 [1-3] (0=返回上层):
```
- 数字绿色加粗（`BOLD_GREEN`），格式 `N)  项`（有 `)`）
- 标题灰色 `--分组--` 格式
- 末项文案统一 `返回上层`（不写 `返回`/`返回上一级`/`返回上一层`）
- prompt 统一 `(0=返回上层)`
- `0` 总是返回上一级；末项（返回上层）点选也返回
- 所有 case 必须含 `[[ "$c" = "0" ]]` 返回分支
