...
| 术语 | 英文 | 定义 |
|------|------|------|
| **ccconfig** | Claude Code Config | 公开的 Claude Code 配置基础设施仓库。包含 init/update/status/monitor 等管理脚本、rules/agents/commands 等配置模板、conftemp/*.example 配置模板。是用户 fork/clone 的入口仓库。不含任何密钥。 |
...
| **第三方 Skill** | Third-Party Skill | 来自外部 GitHub 仓库的 skill（如 mattpocock-skills）。通过 `npx skills add` 安装，自动 symlink 到 `~/.claude/skills/`。由 `conftemp/third-party-skills.txt` 管理清单。 |
