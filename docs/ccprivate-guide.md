...
从 `ccconfig/conftemp/llm.json.example` 复制模板，填入你的 API Key：

```bash
cp ~/git/ccconfig/conftemp/llm.json.example ~/git/ccprivate/conf/llm.json
```
...
```bash
cp ~/git/ccconfig/conftemp/claude.json.example ~/git/ccprivate/conf/claude.json
```
...
```bash
cp ~/git/ccconfig/conftemp/ubuntu.json.example ~/git/ccprivate/conf/ubuntu.json
```
...
每个都有对应的 `.example` 模板在 `ccconfig/conftemp/` 下，复制后编辑即可。
...
`init-ccprivate.sh` 已自动生成 `setup.sh`，无需手动创建。脚本内容：

1. 私有 conf/ → ccconfig/conftemp/ symlink
2. 个人 link/ → ~/  symlink（CLAUDE.md、settings.json、.config.json）
3. projects/ memory 目录 symlink
4. 调用 `ccconfig/setup-links.sh` 处理公开部分

```bash
# 查看生成的 setup.sh
cat ~/git/ccprivate/setup.sh
```
...
`init-skill.sh sync` 自动调用 `apply-config.sh` 建立这些 symlink。

### 添加新配置文件

1. 在 ccconfig 创建 `.example` 模板：`ccconfig/conftemp/new-service.json.example`
...
| 方式 | 格式 | 消费方 | 路径 |
|------|------|--------|------|
| 系统配置 | JSON（conf/*.json） | ccconfig init 脚本 | ccprivate/conf/ → ccconfig/conftemp/ |
...
### Q: 可以不用 ccprivate 吗？
A: 可以。用 `bash ccconfig/bin/init-ccprivate.sh` 交互式配置向导，手动输入 API Key。但 ccprivate 方式更方便——一次配置，多机复用，`git pull` 即可恢复。
