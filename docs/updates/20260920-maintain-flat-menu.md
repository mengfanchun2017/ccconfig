...
| 3 更新 | 6 | 自更新、skill 同步、ccprivate 升级、组件升级(交互/全部)、全量更新 |
| 4 LLM | 8 | 切换/列表/诊断/探测/修 model/sync/heal/删除/bill |
| 5 用量 | 10 | 统计/报告/归档/快照/重算/timer 四件套/pricing |
...
- `_submenu_usage` → `5A–5J`（原顶部状态横幅移到 `5F` 定时器状态）
- `_submenu_getnote` → `8A–8F`
- `_submenu_update_sync` → `3A–3F`
