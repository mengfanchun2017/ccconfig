...
#
# 执行 7 项检测：安装 / 授权 / 创建文档 / Base 写入 / 权限完整性 / larkbridge / API 兼容
#
# 用法：
...
import json,sys
try: d=json.load(sys.stdin); t=d.get('identities',{}).get('bot',{}).get('token','') or d.get('identities',{}).get('user',{}).get('token',''); print(t)
except: pass" 2>/dev/null || true)
    if [ -n "$token" ]; then
      local rc; rc=$(curl -s -o /dev/null -w '%{http_code}' --max-time 10 -X DELETE \
...
echo ""
echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${CYAN}  飞书集成 E2E 测试${NC}"
echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo ""

ACTIVE_INFO=$(_get_active_config)
ACTIVE_NAME="${ACTIVE_INFO%%|*}"
ACTIVE_DIR="${ACTIVE_INFO#*|}"
...
if echo "$ao" | grep -q '"tokenStatus".*"valid"'; then
  local uid; uid=$(echo "$ao" | python3 -c "import json,sys; d=json.load(sys.stdin); u=d.get('identities',{}).get('user',{}); print(u.get('userName','?')+' ('+u.get('openId','?')+')')" 2>/dev/null)
  _ok "OAuth 有效 — $uid"
...
local ts; ts=$(date +%s)
local doc_out; doc_out=$(echo '# 飞书 E2E 测试文档' | _lark "$ACTIVE_DIR" docs +create --api-version v2 --doc-format markdown --as user --title "ccconfig-e2e-${ts}" --content - 2>/dev/null) || true
...
if [ "$doc_ok" = "True" ]; then
  ...
  local dur; dur=$(echo "$doc_out" | python3 -c "import json,sys; d=json.load(sys.stdin); print(d.get('data',{}).get('document',{}).get('url',''))" 2>/dev/null)
  _ok "文档已创建 — $did"
  CLEANUP_DOCS+=("$did")
else
...
" 2>/dev/null) || true
  if [ -z "$bc" ]; then _fail "解析 config.yaml 失败"; else
    ...
    local bo; bo=$(cd /tmp && LARKSUITE_CLI_CONFIG_DIR="$ACTIVE_DIR" PATH="$HOME/.local/bin:$PATH" lark-cli base +record-batch-create --base-token "$bt" --table-id "$tb" --as user --json "@${bfn}" 2>&1 | grep -v '^\[lark-cli\]') || true
    rm -f "$bf"
