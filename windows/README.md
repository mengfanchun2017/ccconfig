...
| 文件 | 执行位置 | 用途 |
|------|---------|------|
| `wslconfig.ps1` | Windows PowerShell（管理员） | 创建 `.wslconfig`，配置 mirrored 网络模式 |
| `wslconfig-sync.sh` | WSL bash | 一键同步：从 WSL 触发 Win 端 ps1 + 提示重启 |
...
在 Win 端切换了代理（Clash/V2Ray 等）发现 WSL 终端弹通知，或者 `.wslconfig` 落后 ccconfig 源（`status.sh [13]` 报红）时：

### 推荐：WSL 一键同步

```bash
bash ccconfig/windows/wslconfig-sync.sh
# 然后择机执行（会关掉当前 WSL 会话）
wsl.exe --shutdown
```

可加 `--shutdown` 让脚本自己 10 秒后触发：

```bash
bash ccconfig/windows/wslconfig-sync.sh --shutdown   # ⚠ 会杀掉当前会话
```

### 手动方式（Win 端 PowerShell）
