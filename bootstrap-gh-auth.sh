...
if gh auth status &>/dev/null; then
    ...
    case "$gh_auth_choice" in
        1)
            ...
            echo ""
            echo -e "  ${BOLD}Repository permissions${NC}（只勾这两项）："
            echo -e "    ${YELLOW}Contents${NC}  : ${GREEN}Read and write${NC}（push/clone 必需）"
            echo -e "    ${YELLOW}Metadata${NC}  : ${GRAY}Read-only${NC}（默认勾选，不能取消）"
            echo ""
            echo -e "  ${BOLD}Account permissions${NC}（只需 SSH Keys，其他 No access）："
            echo -e "    ${YELLOW}SSH Keys${NC} : ${GREEN}Read and write${NC}（ssh clone / gh api user/keys 必需）"
            echo ""
            echo -e "  ${GRAY}为什么不选 classic PAT 的 repo scope：${NC}"
            echo -e "  ${GRAY}  classic repo scope 等同所有私有仓全权，fine-grained 限定单仓 + Contents${NC}"
            echo -e "  ${GRAY}  泄露影响范围更小。建仓已改为手动（init-ccprivate-repo.sh 引导），${NC}"
            echo -e "  ${GRAY}  不需要 classic 的 repo 创建能力。${NC}"
            echo ""
            echo -e "  ${GRAY}Token 存在本地 ~/.config/gh/hosts.yml（600 权限），仅本机可用。${NC}"
